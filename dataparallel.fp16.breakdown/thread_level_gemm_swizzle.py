    ...
    def main(
        ...
        with T.Kernel(
            ...
            A_shared = T.alloc_shared(
                A_shared_shape, dtypeAB, scope=shared_scope
            )
            B_shared = T.alloc_shared(
                B_shared_shape, dtypeAB, scope=shared_scope
            )
            C_shared = T.alloc_shared(
                C_shared_shape, dtypeC, scope=shared_scope
            )
            A_local = T.alloc_fragment(
                ((warp_row_tiles // micro_size_x) * local_size),
                dtypeAB,
                scope="local",
            )
            B_local = T.alloc_fragment(
                ((warp_col_tiles // micro_size_y) * local_size),
                dtypeAB,
                scope="local",
            )
            C_local = T.alloc_fragment(
                (
                    (warp_row_tiles // micro_size_x)
                    * (warp_col_tiles // micro_size_y)
                    * local_size
                ),
                accum_dtype,
                scope="local",
            ...
            for ko in T.serial(0, (K // block_K)):
                ...
                for ki in T.serial(0, (block_K // micro_size_k)):
                    ...
                    for i, j in T.grid(
                        ...
                        T.ptx_mma(
                            ...
                            j * local_size,
                            C_local.data,
                            i * (warp_col_tiles // micro_size_y) * local_size
                            + j * local_size,
                            T.bool(False),
                        )
                        T.ptx_mma(
                ...
                for i, j in T.grid(
                    ...
                    for local_id in T.serial(local_size):
                        ...
                        C_shared[
                            ty * (warp_row_tiles // micro_size_x) + i,
                            tz * (warp_col_tiles // micro_size_y) + j,
                            row,
                            col,
                        ] = C_local[
                            i * (warp_col_tiles // micro_size_y * local_size)
